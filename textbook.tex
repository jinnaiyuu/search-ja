...
このテキストでは探索の主な対象として\define{状態空間問題}{state-space problem}を考える。状態空間問題$P = (S, A, s, T)$は状態の集合$S$、初期状態$s \in S$、ゴール集合$T \in S$、アクション集合$A = {a_1, ....,a_n}$、$a_i : S \rightarrow S$がある。アクションはある状態を次の状態に遷移させる関数である。
...
\begin{tabular}{ccc}
	展開順序  	& プライオリティ 	& 性質 \\  
	幅優先探索 	& $\arg \min_n d(n)$ & ユニットコストドメインだと最初に発見した解が最適解である \\
	深さ優先探索 	& $\arg \max_n d(n)$ & \TODO \\
	ダイクストラ法	& $\arg \min_n g(n)$ & 非負コストドメインで最適解が得られる \\
\end{tabular}
\end{adjustbox}
\end{table}
...
\section{枝刈り}
\label{sec:pruning}
\subsection{解コストの上界による枝刈り}
admissible pruning
 Branch and Bound
 wA* pruning (MSA)

\subsection{状態の新奇性に基づく枝刈り}
