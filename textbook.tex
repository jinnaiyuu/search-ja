...
例えば火星探査車などは宇宙のデータなどとても得られるものではないので、探索に基づくエージェントが用いられている。今後のNASAなどによる宇宙開発でも探索技術が重要であり続けると考えられている。
...
世界を正確に表現することは不可能である。
よって、殆どの問題はより解きやすい問題に{\bf モデル化}され、モデル化された問題を解くことによって解きたい問題を解決するというのが情報科学である。
...
本書が主に扱うモデルは\define{完全情報}{perfect information}かつ\define{決定論的}{deterministic}モデルである(\ref{ch:state-space-problem}~\ref{ch:heuristic-serach-variants}章
)。
...
状態空間問題の例をいくつか紹介する。
グリッド経路探索問題など、応用がありつつ、かつ分かりやすい問題を選んだ。これらの問題はすべてヒューリスティック探索研究でベンチマークとして広く使われているものである。
...
\define{ダイクストラ法}{Dijkstra's Algorithm}は非負コストグラフにおいて最短経路を返すアルゴリズムである\cite{dijkstra1959note}。
ダイクストラ法は$g$値が最も小さいノードを優先して展開するグラフ探索アルゴリズムである。
...
実用するにあたっては、後述するヒューリスティック関数が得られない(難しい)問題においてはとりあえずダイクストラ法を試してみることは有効である。
...
ヒューリスティック関数$h$が任意の$n$に対して$h(n) = h^*(n)$である場合、\define{完璧なヒューリスティック}{Perfect Heuristic}と呼ぶ。完璧なヒューリスティック関数がある場合、
...
図\ref{fig:astar-grid}は\define{マンハッタン距離ヒューリスティック}{Manhattan distance heuristic}によるA*探索である。
{\TODO 図で性能差をわかりやすく}
...
fast-downward \cite{}はプランニング問題を解くstate-of-the-artのプランナーである。
...
% TODO: これらのうちキャッチーなものはコラムとして紹介しよう。


%もちろん解法の知識があればプランナーにそれを組み込むことも可能である。

%現在グラフ探索アルゴリズムは古典的プランニング問題のstate-of-the-artの手法である。


%各ドメインに対してグラフ探索による解法は必ずしも最も効率の良い方法とは限らない。例えばTSPのようなドメインはTSPを解くために考えられた様々な手法があり、そちらのほうが効率的である。
%しかし各ドメインに特化した手法はドメイン特有の知識を必要とする。
%しかしここではグラフ探索として解く

%PDDLによるモデルのメリットは様々な問題に対して統一してグラフ探索
%いろいろな問題に対して
