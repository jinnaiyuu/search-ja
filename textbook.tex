...
\captionlistentry[todo]{why-search: Pearl's Heuristics}
...
\captionlistentry[todo]{why-search rather than machine learning}
...
グラフ探索アルゴリズムは主にThink Rationallyを実現するための技術である。
...
世界を正確に表現することは不可能である。
よって、実際に解きたい問題をより解きやすい問題に{\bf モデル化}し、モデル化された問題を解くことによって解決を図る。

どのように世界をモデルするかは非常に難しい問題である。
モデルを簡単なものにすればするほど解きやすくなるが、簡単で正しいモデルをデザインする・自動生成することは非常に難しい。


本書では、簡単で正しいモデルが与えられていることを前提とする。
%これは非常に大きな仮定であり

% TODO: この辺の話は結構テクニカルなのでChapter 2に？
特に、本書が主に扱うモデルは\define{完全情報}{perfect information}かつ\define{決定論的}{deterministic}モデルである(\ref{ch:state-space-problem}~\ref{ch:heuristic-serach-variants}章
)。
...
非決定論的モデルでは遷移が一意に定まらない。自明に非決定論的モデルは決定論モデルを含む。
...
ダイクストラ法(\ref{sec:dijkstra}章)と比較すると、A*探索はゴール状態までのコストの見積もりを考慮して次に展開するノードを決めている。
