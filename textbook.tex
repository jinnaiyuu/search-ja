...
同じ状態に至る経路がたくさんある
YES: グラフ探索アルゴリズム
NO: 木探索アルゴリズム (e.g. IDA*)
SOSO: IDA* with transposition table

問題の特徴がある程度分かっておりヒューリスティックが作れる
YES: ヒューリスティック探索
NO: ブラインド探索

最適解ではなくある程度良い解なら十分である
YES: 局所探索、wA*
NO: A*

最適解と比べてどのくらい良い解かの保証が必要である
YES: wA*

実行時間が足りない
YES: 局所探索、IDA*、wA*、並列探索

メモリが足りない
YES: 深さ優先探索、IDA*、並列探索、外部メモリ探索、

状態空間に似たような状態がたくさんある
YES: Novelty-based Pruning

最適解のコストの上界が求められる
YES: branch-and-bound

コスト(ノード間距離)は連続値である
YES: 二分木によるプライオリティキュー
NO: nested bucketによるプライオリティキュー

分枝数が大きい
YES: 遅延重複検出、モンテカルロ木探索

いろいろな種類の問題があり、それら全てのソルバーを作らなければならない
YES: PDDL、自動行動計画問題
...
状態が完全には観測できない（POMDP）
YES: Belief space planning (cite kaelbling)
...
この問題を解くためのアルゴリズムが紹介されていない・分からない
YES: \url{ddyuudd@gmail.com}にメールする
...
\define{ダイクストラ法}{Dijkstra's Algorithm}はグラフ探索アルゴリズムの一種であり、$g$値が最も小さいノードを優先して展開するアルゴリズムである \cite{dijkstra1959note}。
...
ダイクストラ法などの情報なし探索は人工知能の文脈よりも\define{組み合わせ最適化}{combinatorial optimization}のための手法として使われることが多い\cite{tarjan1983data}。
...
A*探索はすべてのノードから解の下界が求められる場合にそれをヒューリスティック関数として使うことで解の最適性を満たすことができる。
すべてのノードから解の上界が求められるような問題に対しては分枝限定法 (Branch-and-Bound)が使われることがある。
特に深さ優先分枝限定法 (Depth-First Branch-and-Bound)が木探索ではよく使われる。
分枝限定法の処理は一般的な木探索に加えて枝刈りが行う。
アルゴリズム \ref{alg:branch-and-bound}

疑似コードはオープンリストを用いて説明したが、再帰による実装の方が主流である。

% TODO
\begin{algorithm}
\caption{Branch-and-Bound}
\label{alg:branch-and-bound}
	\Input{node $s$, current bound $B$, weight function $w$, successor generation function $Expand$, goal function $Goal$, upper bound function $U$}
	\Output{Path from $s$ to a goal node $t \in T$, or $\emptyset$ if no such path exists}
	$B \leftarrow \infty$
...
\begin{algorithm}
\caption{B\&B(s, B, path): Branch-and-Bound-Recursion}
\label{alg:branch-and-bound-rec}
	...
	\For {each $v \in Succ(s)$} {
		$B\&B(v, B)$
	}
\end{algorithm}

一般に、アルゴリズムの実行に従って最適解とは限らない解を次々と発見する手法において分枝限定法の考え方が採用できる。すなわち現在発見された中でもっとも良い解
...
まず、初期状態$s_0$だけによる集合$S_0 = \{s_0\}$を考える。これに対して$\phi_{S_{i}}$は集合$S_i$を表す特徴関数だとする。これを用いることで次状態集合を次々と求めるこ
\section{山登り法 (Hill Climbing)}
\label{sec:hill-climbing}
\define{山登り法}{hill climbing}は局所探索アルゴリズムであり、特に組み合わせ最適化問題のためのアルゴリズムとして使われる。
山登り法のアルゴリズムは非常に単純である。子ノードのうち最も$h$値が小さいノードを選ぶことを繰り返す。
...
Qiitaにヒューリスティック探索アルゴリズムの記事\url{http://qiita.com/advent-calendar/2015/graphsearch}がまとまっているのでそちらも参照されたい。

Donald KnuthによるThe Art of Computer Programming Volume 4A Combinatorial Algorithmsは組み合わせ問題としてグラフ探索を扱っている。
