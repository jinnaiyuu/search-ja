...
\label{alg:branch-and-bound}
        \Input{非明示的状態空間グラフ $(u_0, Goal, Expand, w)$、ヒューリスティック関数 $h$}
	\Output{$u_0$からゴール状態への経路、経路が存在しなければ$\emptyset$}
	$U \leftarrow \infty$\;
	$path \leftarrow \emptyset$\;
	$(U, path) \leftarrow DFB\&B(u_0, 0, U, path)$\;
	\Return $path$\;
\end{algorithm}

\begin{algorithm}
\caption{DFB\&B(u, g, U, path): 分枝限定法の再帰計算}
\label{alg:branch-and-bound-rec}
  ...
  \Else {
    $Succ(u) \leftarrow Expand(u)$, sorted according to $h$\;
	  \For {$v \in Succ(u)$} {
	    \If {$g + h(v) < U$} {
	      $(U, path) \leftarrow DFB\&B(v, g + w(u, v), U, path)$\;
	    }
	  }
  }
  \Return $(U, path)$
...
\label{alg:depth-first-iterative-deepening}
  \Input{非明示的状態空間グラフ $(u_0, Goal, Expand, w)$}
	\Output{$s$からゴール状態への経路、経路が存在しなければ$\emptyset$}
        $U' \leftarrow 0$\;
        $path \leftarrow \emptyset$\;
        \While {$path = \emptyset$ {\bf and} $U' \neq \infty$} {
          $U \leftarrow U'$\;
          $(U', path) \leftarrow CLDFS\text{-}DFID(u_0, 0, U)$\;
        }
        \Return $path$\;
\end{algorithm}
...
\label{alg:cldfs}
	\Input{状態 $u$、経路コスト $g$、閾値 $U$}
	\Output{$u$からゴール状態$t \in T$への経路、経路が存在しなければ$\emptyset$}
        $U' \leftarrow \infty$\;
	\If {$Goal(u)$} {
		\Return $(U', Path(u))$\;
	}
	\For {each $v \in Expand(u)$} {
		\uIf {$g + w(u, v) \leq U$} {
			$U', p \leftarrow CLDFS\text{-}DFID(v, g + w(u, v), U)$\;
		  \If {$p \neq \emptyset$} {
        $p' \leftarrow (u, p)$\;
		    \Return $(U', p')$\;
		  }
...
\label{alg:ida}
  \Input{非明示的状態空間グラフ $(u_0, Goal, Expand, w)$、ヒューリスティック$h$}
	\Output{$u_0$からゴール状態への経路、経路が存在しなければ$\emptyset$}
        $U' \leftarrow h(u_0)$\;
        $path \leftarrow \emptyset$\;
        \While {$path = \emptyset$ {\bf and} $U' \neq \infty$} {
          $U \leftarrow U'$\;
          $(U', path) \leftarrow CLDFS\text{-}IDA(u_0, 0, U)$\;
        }
        \Return $path$\;
\end{algorithm}
...
\label{alg:external-brfs}
	\Input{非明示的状態空間グラフ $(u_0, Goal, Expand, w)$}
	\Output{$u_0$からゴール状態$t \in T$への経路、経路が存在しなければ$\emptyset$}
	$Open(-1) \leftarrow \emptyset$\;
	$Open(0) \leftarrow \{u_0\}$\;
	$i \leftarrow 1$\;
	\While {$Open(i-1) \neq \emptyset$}{
		\If {$\exists (u \in Open(i-1)) Goal(u)$} {
			\Return $Path(u)$\;
		}
                
		$R(i) \leftarrow {u | u \in Expand(u'), u' \in Open(i-1)}$\;
    \For {each $v \in R(i)$} {
      $parent(v) \leftarrow u$ s.t. $v \in Expand(u)$\;
    }
...
\label{alg:width-based-search}
	\Input{非明示的状態空間グラフ $(u_0, Goal, Expand, w)$、特徴関数 $h_0, h_1,..., h_{m-1}$、幅$i$}
	\Output{$u_0$からゴール状態への経路、経路が存在しなければ$\emptyset$}
...
\begin{algorithm}
  ...
  \Input{非明示的状態空間グラフ $(u_0, Goal, Expand, w)$、ヒューリスティック関数$h$、ハッシュ関数 $H$}
  \Output{$u_0$からゴール状態への経路、経路が存在しなければ$\emptyset$}
